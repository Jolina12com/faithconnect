    ...
    <div class="container py-5">
        <div class="row justify-content-center">
            <div class="col-lg-10">
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        <h1 class="page-title text-dark">Edit Donation</h1>
                    </div>
                ...
                <div class="donation-card">
                    ...
                    <form action="{{ route('admin.donations.update', $donation->id) }}" method="POST" id="donationForm" class="donation-form">
                        @csrf
                        @method('PUT')
                        
                        <input type="hidden" name="donation_type" id="donation_type" value="{{ old('donation_type', $donation->donation_type) }}">
                        
                        <div class="form-section">
                            <div class="section-header">
                                <i class="fas fa-gift"></i>
                                <h6>What are they donating?</h6>
                            </div>
                            <div class="donation-type-selector">
                                ...
                                <div class="input-wrapper">
                                    <input type="text" 
                                        ...
                                        placeholder="Type donation (e.g., Money, Rice, Blankets, or anything else)"
                                        value="{{ old('donation_input', $donation->donation_type == 'monetary' ? 'Money' : $donation->item_name) }}"
                                        autocomplete="off"
                                        required>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-wrapper">
                                            <input type="text" name="donor_name" id="donor_name" 
                                                ...
                                                value="{{ old('donor_name', $donation->donor_name ?? ($donation->user ? $donation->user->first_name . ' ' . $donation->user->last_name : '')) }}" 
                                                placeholder="Type donor name or select from members"
                                                required>
                                            <i class="fas fa-user input-icon"></i>
                                        </div>
                                        <input type="hidden" name="user_id" id="user_id" value="{{ old('user_id', $donation->user_id) }}">
                                        @error('donor_name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        <small class="text-muted">Type any name or select from registered members</small>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-wrapper amount-input">
                                            ...
                                            <input type="number" name="amount" id="amount" class="form-control @error('amount') is-invalid @enderror" 
                                                value="{{ old('amount', $donation->amount) }}" step="0.01" min="0.01" placeholder="0.00">
                                        </div>
                                        @error('amount')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-wrapper">
                                            <select name="payment_method" id="payment_method" 
                                                ...
                                                <option value="Credit Card" {{ old('payment_method', $donation->payment_method) == 'Credit Card' ? 'selected' : '' }}>💳 Credit Card</option>
                                                <option value="Bank Transfer" {{ old('payment_method', $donation->payment_method) == 'Bank Transfer' ? 'selected' : '' }}>🏦 Bank Transfer</option>
                                                <option value="GCash" {{ old('payment_method', $donation->payment_method) == 'GCash' ? 'selected' : '' }}>📱 GCash</option>
                                                <option value="PayMaya" {{ old('payment_method', $donation->payment_method) == 'PayMaya' ? 'selected' : '' }}>💰 PayMaya</option>
                        ...
                        <div id="physical-fields" class="form-section" style="display: none;">
                            ...
                            <input type="hidden" name="item_name" id="item_name" value="{{ old('item_name', $donation->item_name) }}">
                            
                            <div class="selected-item-display">
                                <i class="fas fa-box-open"></i>
                                <strong id="selected-item-display">Selected Item: {{ $donation->item_name }}</strong>
                            </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-wrapper">
                                            <input type="number" name="quantity" id="quantity" 
                                                ...
                                                value="{{ old('quantity', $donation->quantity) }}" step="0.01" min="0.01" placeholder="0">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-wrapper">
                                            <select name="unit" id="unit" class="form-control @error('unit') is-invalid @enderror">
                                                ...
                                                <option value="kg" {{ old('unit', $donation->unit) == 'kg' ? 'selected' : '' }}>⚖️ Kilograms (kg)</option>
                                                <option value="g" {{ old('unit', $donation->unit) == 'g' ? 'selected' : '' }}>📏 Grams (g)</option>
                                                ...
                                                <option value="cans" {{ old('unit', $donation->unit) == 'cans' ? 'selected' : '' }}>🥫 Cans</option>
                                                <option value="bottles" {{ old('unit', $donation->unit) == 'bottles' ? 'selected' : '' }}>🍼 Bottles</option>
                                                <option value="pairs" {{ old('unit', $donation->unit) == 'pairs' ? 'selected' : '' }}>👫 Pairs</option>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-wrapper">
                                            <input type="date" name="expiry_date" id="expiry_date" 
                                                ...
                                                value="{{ old('expiry_date', $donation->expiry_date) }}" min="{{ date('Y-m-d') }}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-wrapper">
                                            <select name="condition" id="condition" class="form-control @error('condition') is-invalid @enderror">
                                                ...
                                                <option value="new" {{ old('condition', $donation->condition) == 'new' ? 'selected' : '' }}>✨ New</option>
                                                <option value="used" {{ old('condition', $donation->condition) == 'used' ? 'selected' : '' }}>🔄 Used</option>
                                                <option value="good" {{ old('condition', $donation->condition) == 'good' ? 'selected' : '' }}>👍 Good</option>
                                                <option value="fair" {{ old('condition', $donation->condition) == 'fair' ? 'selected' : '' }}>👌 Fair</option>
                                            </select>
                        ...
                        <div class="form-section">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-wrapper">
                                            <input type="text" name="receipt_number" id="receipt_number" 
                                                ...
                                                value="{{ old('receipt_number', $donation->receipt_number) }}" placeholder="RCP-001">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-wrapper">
                                    <textarea name="notes" id="notes" rows="3" 
                                        ...
                                        placeholder="Share any additional details about this donation...">{{ old('notes', $donation->notes) }}</textarea>
                            ...
                            <div class="custom-checkbox">
                                <input type="checkbox" name="is_recurring" id="is_recurring" value="1" 
                                    {{ old('is_recurring', $donation->is_recurring) ? 'checked' : '' }}>
                                <label for="is_recurring">
                                    <span class="checkmark"></span>
                                    <span class="checkbox-text">
                                        <strong>Recurring Donation</strong>
                                        <small>This is a regular donation</small>
                                    </span>
                                </label>
                                @error('is_recurring')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="form-actions">
                            <a href="{{ route('admin.donations.index') }}" class="btn btn-secondary me-3">
...
$(document).ready(function() {
    let selectedType = '{{ old('donation_type', $donation->donation_type) }}';
    let selectedItemName = '{{ old('item_name', $donation->item_name) }}';
    ...
    $('#donation_input').autocomplete({
        source: function(request, response) {
            ...
            $.ajax({
                url: '{{ route('admin.donations.items.autocomplete') }}',
                data: { term: term },
    ...
    $('#donor_name').autocomplete({
        source: function(request, response) {
            ...
            donorSearchTimeout = setTimeout(() => {
                $.ajax({
                    url: '{{ route('admin.donations.donors.autocomplete') }}',
    ...
    // Category autocomplete with caching
    let categoryCache = [];
    ...
    $('#category').autocomplete({
        source: function(request, response) {
            ...
            if (categoryCacheLoaded) {
                const filtered = categoryCache.filter(cat => 
            ...
            $.ajax({
                url: '{{ route('admin.donations.categories.autocomplete') }}',
                data: { term: term },
                success: function(data) {
                    if (!categoryCacheLoaded) {
                        categoryCache = data;
    ...
    $('#donationForm').on('submit', function(e) {
        ...
        if (!inputValue) {
            e.preventDefault();
            alert('Please enter what is being donated');
